...
async function renderSummary(input: VatDossierInput): Promise<Uint8Array> {
  ...
  page.drawText(`Documentos: ${input.invoices.length}`, { x: 48, y: 680, size: 11, font: body });
  page.drawText(`Base: ${base.toFixed(2)} EUR`, { x: 48, y: 655, size: 11, font: body });
  page.drawText(`Cuota: ${tax.toFixed(2)} EUR`, { x: 48, y: 630, size: 11, font: body });
  page.drawText('Resumen de apoyo para asesoría. No genera ni presenta el modelo 303.', {
    x: 48,
    y: 80,
    size: 9,
    font: body,
...
export async function createVatDossier(input: VatDossierInput): Promise<VatDossierResult> {
  if (
    input.issues.some((issue) => issue.severity === 'BLOCKING' && issue.status === 'OPEN')
  ...
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(input.invoices), 'Facturas');
...
export function readVatDossierFile(zipBytes: Uint8Array, filename: string): Uint8Array | null {
  return unzipSync(zipBytes)[filename] ?? null;
}

export function readVatDossierJsonFile<T = unknown>(zipBytes: Uint8Array, filename: string): T | null {
...
export function verifyVatDossier(zipBytes: Uint8Array): boolean {
  ...
  return Object.entries(parsed.files).every(([name, hash]) => (
    files[name]
