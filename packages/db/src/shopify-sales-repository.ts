...
export class DrizzleShopifySalesRepository<TQueryResult extends PgQueryResultHKT> {
  ...
  async list(input: ShopifySalesFilters) {
    ...
    if (input.settlementStatus === 'PENDING') filters.push(sql`${commercialOrders.totalAmount} <> 0 and not exists (select 1 from ${shopifyPaymentsLedgerEntries} le where le.tenant_id = ${commercialOrders.tenantId} and (le.commercial_order_id = ${commercialOrders.id} or le.shopify_order_name = ${commercialOrders.externalOrderId}) and le.external_payout_id is not null)`);
    if (input.settlementStatus === 'SETTLED') filters.push(sql`exists (select 1 from ${shopifyPaymentsLedgerEntries} le where le.tenant_id = ${commercialOrders.tenantId} and (le.commercial_order_id = ${commercialOrders.id} or le.shopify_order_name = ${commercialOrders.externalOrderId}) and le.external_payout_id is not null)`);
    ...
    const [items, [totalRow], [metrics]] = await Promise.all([
      this.db.select({
        ...
        transactionCount: sql<number>`(select count(*) from ${shopifyOrderPaymentEvents} pe where pe.tenant_id = ${commercialOrders.tenantId} and (pe.commercial_order_id = ${commercialOrders.id} or pe.shopify_order_name = ${commercialOrders.externalOrderId}))`,
        ledgerCount: sql<number>`(select count(*) from ${shopifyPaymentsLedgerEntries} le where le.tenant_id = ${commercialOrders.tenantId} and (le.commercial_order_id = ${commercialOrders.id} or le.shopify_order_name = ${commercialOrders.externalOrderId}))`,
        feeAmount: sql<string>`coalesce((select sum(le.fee_amount) from ${shopifyPaymentsLedgerEntries} le where le.tenant_id = ${commercialOrders.tenantId} and (le.commercial_order_id = ${commercialOrders.id} or le.shopify_order_name = ${commercialOrders.externalOrderId})), 0)`,
        payoutStatus: sql<string | null>`(select case when ${commercialOrders.totalAmount} = 0 then 'LEDGER_NOT_REQUIRED' when max(le.external_payout_id) is not null then 'SETTLED' when count(*) > 0 then 'PENDING' else 'LEDGER_MISSING' end from ${shopifyPaymentsLedgerEntries} le where le.tenant_id = ${commercialOrders.tenantId} and (le.commercial_order_id = ${commercialOrders.id} or le.shopify_order_name = ${commercialOrders.externalOrderId}))`,
      ...
      this.db.select({
        ...
        pendingSettlement: sql<number>`count(*) filter (where ${commercialOrders.totalAmount} <> 0 and not exists (select 1 from ${shopifyPaymentsLedgerEntries} le where le.tenant_id = ${commercialOrders.tenantId} and (le.commercial_order_id = ${commercialOrders.id} or le.shopify_order_name = ${commercialOrders.externalOrderId}) and le.external_payout_id is not null))`,
  ...
  async getById(tenantId: string, orderId: string) {
    ...
    const [lines, transactions, ledger, links, [operation]] = await Promise.all([
      ...
      this.db.select().from(shopifyOrderPaymentEvents).where(and(eq(shopifyOrderPaymentEvents.tenantId, tenantId), sql`(${shopifyOrderPaymentEvents.commercialOrderId} = ${orderId} or ${shopifyOrderPaymentEvents.shopifyOrderName} = ${order.externalOrderId})`)).orderBy(asc(shopifyOrderPaymentEvents.occurredAt)),
      this.db.select().from(shopifyPaymentsLedgerEntries).where(and(eq(shopifyPaymentsLedgerEntries.tenantId, tenantId), sql`(${shopifyPaymentsLedgerEntries.commercialOrderId} = ${orderId} or ${shopifyPaymentsLedgerEntries.shopifyOrderName} = ${order.externalOrderId})`)).orderBy(asc(shopifyPaymentsLedgerEntries.transactionAt)),
    ...
    return { order, lines, transactions, ledger, links, operation: operation ?? null, taxDecision: decision[0] ?? null, documents, audit, eligibility: { hasFiscalConfiguration: Boolean(configuration), hasFiscalProfile: Boolean(profile), hasOrderEvidence: true, hasTransactionsEvidence: isZeroAmount || transactions.length > 0, hasLedgerEvidence: isZeroAmount || ledger.length > 0, hasTaxDecision: decision.length > 0 }, settlement: isZeroAmount ? 'LEDGER_NOT_REQUIRED' : ledger.length === 0 ? 'LEDGER_MISSING' : ledger.some((entry) => entry.externalPayoutId) ? 'SETTLED' : 'PAYOUT_PENDING' };
  }
}
