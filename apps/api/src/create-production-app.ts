...
import {
  ...
  DrizzleCommercialOrdersRepository,
  DrizzleDashboardSummaryRepository,
  DrizzleFinancialEventsRepository,
  DrizzleExpensesRepository,
  DrizzleFiscalConfigurationRepository,
  DrizzleFiscalDocumentsRepository,
  DrizzleImportPreviewRepository,
  DrizzleIssuesRepository,
  DrizzleLegalEntitiesRepository,
  DrizzleOperationsRepository,
  DrizzlePeriodClosesRepository,
  DrizzlePeriodReadinessRepository,
  DrizzleReconciliationRepository,
  DrizzleRoyaltyRepository,
  DrizzleShopifyEvidenceLinksRepository,
  DrizzleShopifyOrderPaymentEventsRepository,
  DrizzleShopifyPaymentsLedgerRepository,
  DrizzleShopifySalesRepository,
  DrizzleSifEventsRepository,
  DrizzleSystemAlertsRepository,
  DrizzleTaxDecisionsRepository,
  DrizzleVatDossiersRepository,
  DrizzleVerifactuSubmissionsRepository,
  ensureDevelopmentTenant,
  migrateOfflineDatabase,
} from "@anclora/db";
import { resolve } from "node:path";
import { buildApp } from "./build-app.js";
import {
  ImportMetadataCipher,
  ImportPreviewPersistenceService,
  type FiscalPersistencePort,
  type ImportPreviewPersistencePort,
} from "./import-preview-persistence.js";
...
import type { ExpensesRepositoryPort } from "./expenses-controller.js";
...
import type {
  CommercialOrdersDedupPort,
  FinancialEventsDedupPort,
  RoyaltyDedupPort,
...
export async function createProductionApp() {
  ...
  const blobStoreConnected = Boolean(
  ...
  let expensesRepository: ExpensesRepositoryPort;
  let vatDossiersRepository: VatDossiersRepositoryPort;
  let sifEventsRepository: SifEventsRepositoryPort;
  let systemAlertsRepository: SystemAlertsRepositoryPort & {
    open(input: {
      tenantId: string;
      severity: string;
      type: string;
      ...
      eventType?: "INTEGRITY_ERROR" | "SUBMISSION_ERROR" | "ANOMALY";
    }): Promise<unknown>;
    report(input: {
      tenantId: string;
      dossierId: string;
  ...
  let recordOperationalSif: (input: {
    tenantId: string;
    eventType:
  ...
  if (process.env.DATABASE_URL) {
    ...
    const commercialOrdersRepositoryForMatching =
      new DrizzleCommercialOrdersRepository(database.db);
    const financialEventsRepositoryForMatching =
      new DrizzleFinancialEventsRepository(database.db);
    const legalEntitiesRepositoryForMatching =
      new DrizzleLegalEntitiesRepository(database.db);
    const royaltyRepositoryForPersistence = new DrizzleRoyaltyRepository(
      database.db,
    );
    const fiscalConfigurationRepositoryForTax =
      new DrizzleFiscalConfigurationRepository(database.db);
    const shopifyOrderPaymentEventsRepositoryForPersistence =
      new DrizzleShopifyOrderPaymentEventsRepository(database.db);
    const shopifyPaymentsLedgerRepositoryForPersistence =
      new DrizzleShopifyPaymentsLedgerRepository(database.db);
    const shopifyEvidenceLinksRepositoryForPersistence =
      new DrizzleShopifyEvidenceLinksRepository(database.db);
    ...
    const fiscalDocumentsRepositoryForMatching =
      new DrizzleFiscalDocumentsRepository(database.db);
    ...
    const importPreviewRepository = new DrizzleImportPreviewRepository(
      database.db,
    );
    ...
    operationsRepository = new DrizzleOperationsRepository(database.db);
    commercialOrdersRepository = commercialOrdersRepositoryForMatching;
    financialEventsRepository = new DrizzleFinancialEventsRepository(
      database.db,
    );
    ...
    periodReadinessRepository = new DrizzlePeriodReadinessRepository(
      database.db,
    );
    expensesRepository = new DrizzleExpensesRepository(database.db);
    ...
    systemAlertsRepository = new DrizzleSystemAlertsRepository(
      database.db,
      sifRepository,
    );
    recordOperationalSif = (input) => sifRepository.record(input);
    recordStartup = (deploymentId) =>
      sifRepository.recordStartupForAll(deploymentId);
    const verifactuRepository = new DrizzleVerifactuSubmissionsRepository(
      database.db,
    );
    verifactuSubmissionsRepository = verifactuRepository;
    verifactuDueRepository = verifactuRepository;
    dashboardSummaryRepository = new DrizzleDashboardSummaryRepository(
      database.db,
    );
    authService = new AuthService(
      new ConfiguredIdentityProvider(process.env.AUTH_IDENTITIES_JSON),
      new DrizzleAuthAuditRepository(database.db),
    );
    ...
    const database = createOfflineDatabase(
      process.env.OFFLINE_DATABASE_PATH ??
        resolve(process.cwd(), ".data/anclora-fiscal"),
    );
  ...
  const app = await buildApp({
    ...
    expensesRepository,
    expenseCipher: new ImportMetadataCipher(metadataSecret),
  ...
  const processDueHandler = async (
    request: FastifyRequest,
    reply: FastifyReply,
  ) => {
    const secret =
      process.env.CRON_SECRET ?? process.env.VERIFACTU_INTERNAL_TOKEN;
    ...
    const summary = await processDueVerifactuSubmissions({
      ...
      onOutcome: async ({ tenantId, submissionId, status }) => {
        if (status === "ACCEPTED_WITH_ERRORS" || status === "REJECTED") {
          await recordOperationalSif({
            tenantId,
            eventType: status,
            actor: "verifactu-cron",
            detail: { submissionId },
          });
        } else if (status === "RETRY_SCHEDULED") {
          await recordOperationalSif({
            tenantId,
            eventType: "RESTORE_RETRY",
            actor: "verifactu-cron",
            detail: { submissionId },
          });
        }
        if (status === "REJECTED") {
          await systemAlertsRepository.open({
            tenantId,
            severity: "CRITICAL",
            type: "VERIFACTU_REJECTED",
            source: "verifactu-cron",
            detail: { submissionId },
            deduplicationKey: `verifactu-rejected:${submissionId}`,
