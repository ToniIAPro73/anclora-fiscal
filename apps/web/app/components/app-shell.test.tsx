...
describe('AppShell', () => {
  it('renders every enabled nav item as a link', () => {
    ...
    expect(
      screen.getByRole('link', { name: /Centro de control/ }),
    ).toHaveAttribute('href', '/');

    expect(
      screen.getByRole('link', { name: /Importaciones/ }),
    ).toHaveAttribute('href', '/imports');

    expect(
      screen.getByRole('link', { name: /Ventas Shopify/ }),
    ).toHaveAttribute('href', '/sales/shopify');

    expect(
      screen.getByRole('link', { name: /Facturación/ }),
    ).toHaveAttribute('href', '/invoicing');

    expect(
      screen.getByRole('link', { name: /Reglas fiscales/ }),
    ).toHaveAttribute('href', '/tax-rules');

    expect(
      screen.getByRole('link', { name: /Periodos fiscales/ }),
    ).toHaveAttribute('href', '/tax-periods');
  });
  ...
  it('disables comingSoon items instead of linking to them', () => {
    ...
    expect(
      screen.queryByRole('link', { name: /Liquidaciones KDP/ }),
    ).not.toBeInTheDocument();

    expect(
      screen.queryByRole('link', { name: /Registros/ }),
    ).not.toBeInTheDocument();

    const disabledItem = screen
      .getByText('Liquidaciones KDP')
      .closest('.nav-item-disabled');

    expect(disabledItem).toHaveAttribute('aria-disabled', 'true');

    expect(
      screen.getAllByText('Próx.').length,
    ).toBeGreaterThan(0);
  });
  ...
  it('shows the pending-count badge for items with pendingCountKey data', () => {
    ...
    render(
      <AppShell
        pendingCounts={{ reconciliationTotal: 4 }}
        hasPayoutData
      >
        content
      </AppShell>,
  ...
  it('hides Conciliación when hasPayoutData is false', () => {
    ...
    expect(
      screen.queryByRole('link', { name: /Conciliación/ }),
    ).not.toBeInTheDocument();
  ...
  it('shows Conciliación when hasPayoutData is true', () => {
    ...
    expect(
      screen.getByRole('link', { name: /Conciliación/ }),
  ...
  it('defaults to hiding Conciliación and fetches the summary when hasPayoutData is not provided', async () => {
    ...
    expect(
      await screen.findByRole('link', { name: /Conciliación/ }),
    ).toHaveAttribute('href', '/reconciliation');
